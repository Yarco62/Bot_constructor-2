def start():
    bot_creating(res, req, token, file_name)
...
def make_button(name, message):
    names = ''
    button = f'''    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
'''
        ...
        with open(file_name + '.py', 'w+') as f:
            ...
            for line in instructions:
                if line.split()[0] == 'token':
                    ...
                    on_start_message = line.strip().split('start_message')[1]
            ...
            f.write(start_message(on_start_message))
